...
describe("estruturarProposta — proposta com faixa de preço", () => {
  it("860 docs em 48h → 2 agentes, faixa base ×1.45", async () => {
    const c = await estruturarProposta("tenho 860 notas fiscais e contratos pra organizar", false);
  ...
  it("até 500 docs → 1 agente", async () => {
    const c = await estruturarProposta("tenho 86 notas fiscais pra organizar", false);
    expect(c.agentesSugeridos).toBe(1);
  });

  it("SLA urgente (24h) amplia o teto para ×1.75", async () => {
    const c = await estruturarProposta("preciso disso urgente, umas 300 notas", false);
  ...
  it("sem volume mencionado → 2 agentes e teto ×1.6 (mais incerteza)", async () => {
    const c = await estruturarProposta("organiza meus contratos e boletos", false);
    expect(c.agentesSugeridos).toBe(2);
  ...
  it("áudio sem texto → estrutura genérica, 2 agentes, faixa 830–1490", async () => {
    const c = await estruturarProposta("", true);
    expect(c.agentesSugeridos).toBe(2);
  ...
  it("skillsDetectadas nunca é menor que 1", async () => {
    const c = await estruturarProposta("organiza minha papelada", false);
